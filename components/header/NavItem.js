...
const NavItem = ({navitem,mobile,setDrawerOpen}) => {
        ...
        <>
            {
                ...
                (<li className="relative" ref={dropdown} onClick={() => toggle(!open)}>
                    <span className="block lg:inline">
                        <a className="block border-b-2 border-cyan-900 lg:border-0 lg:inline-block px-4 h-14 leading-[56px] dark:bg-slate-800 hover:bg-slate-700 hover:text-slate-200 transition-all duration-200 cursor-pointer font-semibold">{navitem.title}{navitem.dropdowns && <><CaretDownIcon className="hidden lg:inline-block mb-1 ml-1 pointer-events-none" />{open ? <MinusIcon className="inline-block lg:hidden float-right h-14 pointer-events-none" /> : <PlusIcon className="inline-block lg:hidden float-right h-14 pointer-events-none" />}</>}
                        </a>
                    </span>
                    {open && <Dropdown items={navitem.dropdowns} open={open} mobile={mobile} setDrawerOpen={setDrawerOpen} />}
                </li>) :
                (<li>
                    <Link href={`/${navitem.slug}`}>
                        <a className="block lg:inline-block border-b-2 border-cyan-900 lg:border-0 px-4 h-14 leading-[56px] dark:bg-slate-800 hover:bg-slate-700 hover:text-slate-200 transition-all duration-200 font-semibold" onClick={ () => {
